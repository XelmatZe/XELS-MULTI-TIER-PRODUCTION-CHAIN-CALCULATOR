 ...
 * This avoids CORS issues in the browser and reduces load on the upstream API.
 *
...
export default async function handler(req, res) {
  // Allow all origins (public tool)
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Methods", "GET");

  // Cache for 1 hour at the CDN edge, 55 minutes in browser
  ...
  try {
    ...
    res.status(502).json({ error: "Failed to fetch prices from upstream", detail: error.message });
  }
}
